...
export type NeuralTrainingOptions = {
  ...
  rlCounterfactualMinBehaviorWins?: number;
  ...
  for (let scanIndex = 0; scanIndex < counterfactualScanCount; scanIndex++) {
    ...
    rollout.transitions.forEach((transition, transitionIndex) => {
      ...
      if (creditMode === "counterfactual") {
        ...
        if (
          options.counterfactualTrainingMode !== "policy_gradient" &&
          getSafeWinCountThreshold(options.counterfactualMinBehaviorWins) > 0 &&
          result.behaviorWinCount <
            getSafeWinCountThreshold(options.counterfactualMinBehaviorWins)
        ) {
          counterfactualBehaviorWinRateSkippedCount += 1;
          return;
        }
        ...
        if (
          options.counterfactualTrainingMode !== "policy_gradient" &&
          options.counterfactualRequireSameMoveType &&
          !isSameCounterfactualMoveTypePair(movePair)
        ) {
          counterfactualMoveTypeMismatchSkippedCount += 1;
          return;
        }
        if (
          options.counterfactualTrainingMode !== "policy_gradient" &&
          options.counterfactualRequireDifferentMoveType &&
          isSameCounterfactualMoveTypePair(movePair)
        ) {
          counterfactualMoveTypeMatchSkippedCount += 1;
          return;
        }
        if (
          options.counterfactualTrainingMode !== "policy_gradient" &&
          options.counterfactualMinScoreReturnGap > 0 &&
          !doesCounterfactualBestMeetScoreReturnGap(
            transition,
            result,
            options.counterfactualMinScoreReturnGap
          )
        ) {
          counterfactualScoreReturnGapSkippedCount += 1;
          return;
        }
        if (
          options.counterfactualTrainingMode !== "policy_gradient" &&
          options.counterfactualMinPounceProgressGap > 0 &&
          !doesCounterfactualBestMeetPounceProgressGap(
            transition,
            result,
            options.counterfactualMinPounceProgressGap
          )
        ) {
          counterfactualPounceProgressGapSkippedCount += 1;
          return;
        }
        if (
          options.counterfactualTrainingMode !== "policy_gradient" &&
  ...
  for (let scanIndex = 0; scanIndex < counterfactualScanCount; scanIndex++) {
    ...
    rollout.transitions.forEach((transition, transitionIndex) => {
      ...
      if (
        ...
        moveTypeMismatchSkippedCount += 1;
        return;
      }
      if (
        options.counterfactualTrainingMode !== "policy_gradient" &&
      ...
      if (
        ...
        pounceProgressGapSkippedCount += 1;
        return;
      }
      if (
        options.counterfactualTrainingMode !== "policy_gradient" &&
...
function doesCounterfactualBestMeetScoreReturnGap(
  transition: RolloutTransition,
  result: CounterfactualTransitionResult,
  minGap: number
): boolean {
  ...
  return (
    getCounterfactualBestCandidate(result).rolloutScoreReturn -
      behavior.rolloutScoreReturn >=
    minGap
  );
}

function doesCounterfactualBestMeetPounceProgressGap(
  transition: RolloutTransition,
  result: CounterfactualTransitionResult,
  minGap: number
): boolean {
  ...
  return (
    getCounterfactualBestCandidate(result).rolloutPounceProgressReturn -
      behavior.rolloutPounceProgressReturn >=
    minGap
  );
}

function getCounterfactualBehaviorCandidate(
  transition: RolloutTransition,
  result: CounterfactualTransitionResult
): CounterfactualCandidateReturn | null {
  ...
  return (
    result.candidates.find(
      (candidate) => candidate.candidate.key === behavior.key
    ) ?? null
  );
}
...
function isSameCounterfactualMoveTypePair(movePair: string): boolean {
  const [winnerType, behaviorType] = movePair.split(">");
  return (
    winnerType != null &&
    behaviorType != null &&
    normalizeMovePair(winnerType) === normalizeMovePair(behaviorType)
  );
}
...
function getCounterfactualTransitionResult(
  ...
  const behaviorWinStats = greedyCandidate
    ? getCounterfactualReturnGapWinStats(bestCandidate, greedyCandidate)
    : { winRate: 0, winCount: 0, comparisonCount: 0 };
  ...
  return {
    ...
    behaviorGapStandardError: greedyCandidate
      ...
      : 0,
    behaviorWinRate: behaviorWinStats.winRate,
...
function getSafeWinCountThreshold(value: number): number {
  if (!Number.isFinite(value)) {
    return 0;
  }
...
function getCounterfactualReturnGapWinStats(
  winner: CounterfactualCandidateReturn,
  loser: CounterfactualCandidateReturn
): { winRate: number; winCount: number; comparisonCount: number } {
  const count = Math.min(
    winner.rolloutObjectiveReturns.length,
    loser.rolloutObjectiveReturns.length
  );
  if (count === 0) {
    return { winRate: 0, winCount: 0, comparisonCount: 0 };
  }
  if (winner.candidate.key === loser.candidate.key) {
    return { winRate: 1, winCount: count, comparisonCount: count };
  }
  let winCount = 0;
  ...
  return { winRate: winCount / count, winCount, comparisonCount: count };
}
