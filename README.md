...
default so old baselines and smoke checks remain comparable; set
`RL_INCLUDE_WAIT_ACTIONS=true` and/or `RL_INCLUDE_PREMOVE_ACTIONS=true` for runs
that should train those parts of the expanded surface. Full-deck flip is enabled
by default; set
`RL_INCLUDE_FLIP_DECK_ACTIONS=false` to compare against the old cycle-only stock
...
minimum number of center plays needed before the card can land, with empty piles
counting as a fresh ace path.
...
`Alex 75%` ahead on average point differential by `+0.105` with a `95%` CI of
`+0.021` to `+0.189`, and on raw score by `+0.070` with a `95%` CI of `+0.014`
to `+0.126`. The full-table score-win-share delta was also positive but tiny
(`+0.51pp`, `95%` CI `+0.10pp` to `+0.93pp`), and the head-to-head score share
was `50.27%` with a `95%` CI of `50.02%` to `50.53%`.
...
`RL_COUNTERFACTUAL_PAIRWISE_STOP_MARGIN` skips the primary supervised pairwise
update once the accepted winner already leads the loser by the requested
temperature-normalized score margin. Negative values disable the guard. A value
of `0` only trains a label until the rollout winner has caught the current
behavior action; larger small values such as `0.25` allow a modest deployment
margin without the broad over-push seen from tiny high-confidence label batches.
...
`RL_COUNTERFACTUAL_INCLUDE_MOVE_PAIRS` accepts a comma-separated allow-list such
as `c2s>cycle` and limits supervised counterfactual labels to those
winner-vs-behavior move-type pairs. It is useful for focused follow-up runs after
an audit finds one promising tactical family.
...
Cycle-specific state context and a pairwise stop-margin trust region narrowed
that failure mode but still did not produce a promotable checkpoint. The feature
set now includes cycle-only context for own pounce count, current points, point
differential, opponent minimum pounce count, and opponent pounce pressure; old
checkpoints expand with zero weights on those inputs. A hot tactical run with
that context accepted 6 validated near-policy `cycle>c2s` labels, flipped all 6
label states, and pushed accepted-label winner margins to `+0.782`, but its
1,536-game paired gate was only `+0.016 +/- 0.087` point differential with
negative raw score. `RL_COUNTERFACTUAL_PAIRWISE_STOP_MARGIN=0` was much more
controlled, flipping only 1 of 6 label states; its first 1,536-game gate looked
positive (`+0.0525 +/- 0.0098`), and its divergence trace changed only 16 of
768 paired games with `+2.875` average point differential on those divergent
hands, but the 3,072-game confirmation regressed to `-0.0158 +/- 0.0352`.
`RL_COUNTERFACTUAL_PAIRWISE_STOP_MARGIN=0.25` landed between those extremes,
accepting 7 labels and flipping 3 label states; its first gate was significant
(`+0.1085 +/- 0.0697` over 1,536 games), but the 3,072-game confirmation again
softened to `-0.0161 +/- 0.0409`. The current read is that near-policy
stock-delay labels are real but too sparse for aggregate improvement by
themselves; the next improvement path should either broaden the validated label
distribution or target a higher-frequency tactical family.
